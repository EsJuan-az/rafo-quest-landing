...
const drawAmorEnLosTiempos = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.a)
    ...
    .setHexClass(meshes.hex.bridgeFor.a)
    .doHexLine(1, Facing.SE)
    .doHexRamp(1, Facing.S)
    .doHexRamp(1, Facing.S, false)
    .setHexClass(meshes.hex.way.a)
    .doHexCurve(2, Facing.SW)
    .fixPathEnd()
    .setHexClass(meshes.hex.grass.a) // 0-25 grass a
    .doHexLine(1, Facing.SE, 1) // [0]
    .doHexCurve(2, Facing.N, true, 1, 1) // [1, 2]
    .doHexLine(1, Facing.NE, 4) // [3]
    .doHexLine(2, Facing.SE, 4) // [4, 5]
    .doHexLine(2, Facing.NW, 8) // [6, 7]
    .doHexLine(2, Facing.S, 8) // [8, 9]
    .doHexCurve(2, Facing.S, false, 1, 10) // [10, 11]
    .doHexLine(1, Facing.NW, 11) // [12]
    .doHexCurve(3, Facing.NW, true, 1, 15) // [13, 14, 15]
    .doHexLine(1, Facing.NE, 15) // [16]
    .doHexLine(2, Facing.SE, 15) // [17, 18]
    .doHexCurve(3, Facing.NW, true, 1, 20) // [19, 20, 21]
    .doHexCurve(3, Facing.SE, false, 1, 31) // [22, 23, 24]
    .doHexCurve(2, Facing.SW, false, 1, 33) // [25, 26]
    .setHexClass(meshes.hex.water.a) // 26-37 water a
...
const drawListaDeSchindler = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.b)
    ...
    .fixPathEnd()
    .setHexClass(meshes.hex.grass.b)
    ...
    .setHexClass(meshes.hex.decoWay.b)
    ...
    .setHexClass(meshes.hex.decoWay.b2)
...
const drawMagoFrugal = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.c)
    .doHexCurve(2, Facing.SE)
    .doHexCurve(2, Facing.NE)
    .doHexLine(2, Facing.NW)
    ...
    .doHexLine(1, Facing.NW)
    .fixPathEnd()
    .setHexClass(meshes.hex.grass.c)
    ...
    .setHexClass(meshes.hex.water.a)
...
const drawClubPelea = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.d)
    .doHexCurve(2, Facing.NW, true)
    .doHexCurve(2, Facing.NW, true)
    ...
    .setHexClass(meshes.hex.decoWay.d)
...
const drawGuardianCenteno = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.e)
    .doHexLine(1, Facing.SW)
    ...
    .setHexClass(meshes.hex.decoWay.e)
...
const drawDorian = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.f)
    ...
    .doHexCurve(4, Facing.SE, true)
    .fixPathEnd()
    .setHexClass(meshes.hex.decoWay.f)
...
const drawSiddharta = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.g)
    .doHexLine(2, Facing.SW)
    ...
    .doHexLine(1, Facing.SW, 4)
    .setHexClass(meshes.hex.decoWay.g)
...
const drawLolita = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.h)
    ...
    .setHexClass(meshes.hex.decoWay.h)
...
const drawPedroParamo = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.i)
    .doHexCurve(2, Facing.SE, true)
    .doHexCurve(2, Facing.SE, false, 2)
    .doHexCurve(2, Facing.SE, true)
    ...
    .setHexClass(meshes.hex.decoWay.i)
    .doHexCurve(2, Facing.NE, false, 1, 3)
...
const drawProblemaCuerpos = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.j)
    ...
    .setHexClass(meshes.hex.decoWay.j)
...
const drawMetamor = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.way.d)
    .doHexLine(4, Facing.NE)
    .doHexLine(2, Facing.N)
    .fixPathEnd()
    .setHexClass(meshes.hex.grass.a)
...
const drawResplandor = (hp: HexPath) => {
  const { meshes } = hp.Game;
  hp.setHexClass(meshes.hex.decoWay.b)
    .doHexCurve(2, Facing.NE, true)
    .doHexLine(2, Facing.NE)
    .doHexLine(1, Facing.N)
    .doHexLine(3, Facing.NW)
    ...
    .setHexClass(meshes.hex.grass.l)
